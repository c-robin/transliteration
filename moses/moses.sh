...
$IRSTLM/bin/build-lm.sh -i translit_$LABEL.sb.out -t ./tmp \
    -p -s improved-kneser-ney -o translit_$LABEL.lm.out
...
$MOSES_DIR/scripts/training/train-model.perl -root-dir \
    ...
    out -alignment grow-diag-final-and -reordering msd-bidirectional-fe -lm \
...
$MOSES_DIR/scripts/generic/multi-bleu.perl -lc $TEST_TRUTH < $TEST_OUTPUT
fi
